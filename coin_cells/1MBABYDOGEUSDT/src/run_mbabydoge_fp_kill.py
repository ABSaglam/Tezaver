...
def run_002():
    print("::: RUN 002: FP KILL (1MBABYDOGEUSDT) :::")
    ...
    # Copy Rules
    for f in ["1mbabydogeusdt_fail_rules.json", "1mbabydogeusdt_intraday_cancel_rules.json", "1mbabydogeusdt_success_corridor.json"]:
    ...
    trig_map = {
        ...
        "rule_note": "BABYDOGE_SPECIFIC: Count>=11 (Separates 12-02 TP@11 from strongest FP@10)"
    }
    
    with open(RUN_002 / "rules/1mbabydogeusdt_trigger_map.json", "w") as f:
        json.dump(trig_map, f, indent=4)
        
    # Load Data
    bundle = utils_io.get_data_bundle("1MBABYDOGEUSDT")
    ...
    # Load Rules
    with open(RUN_002 / "rules/1mbabydogeusdt_fail_rules.json") as f: fail = json.load(f)
    with open(RUN_002 / "rules/1mbabydogeusdt_success_corridor.json") as f: corr = json.load(f)
    with open(RUN_002 / "rules/1mbabydogeusdt_intraday_cancel_rules.json") as f: cancel = json.load(f)
    ...
    df_res[v17_schema.V17_COLUMNS].to_csv(RUN_002 / "reports/1MBABYDOGEUSDT_V17_TEST_DAILY.csv", index=False)
    df_res[df_res['Decision']=='NET_ADAY'][v17_schema.V17_COLUMNS].to_csv(RUN_002 / "reports/1MBABYDOGEUSDT_NET_ADAY_REPORT.csv", index=False)
    df_log.to_csv(RUN_002 / "reports/1MBABYDOGEUSDT_SIMULATION_STAGE_LOG.csv", index=False)
    
    # Audit MD
    try:
        r1 = pd.read_csv(RUN_001 / "reports/1MBABYDOGEUSDT_NET_ADAY_REPORT.csv")
    ...
    md = "# 1MBABYDOGEUSDT RUN 002 AUDIT (COUNT>=11)\n\n"
    ...
    with open(RUN_002 / "reports/1MBABYDOGEUSDT_FP_KILL_AUDIT.md", "w") as f:
        f.write(md)
