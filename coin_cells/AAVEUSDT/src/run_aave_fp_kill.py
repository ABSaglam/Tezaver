...
def get_15m_metrics(df_15m, target_date):
    ...
    if count > 0:
        ...
        for t in times:
            ...
            # FT2 (Single bar lookahead ~30m equivalent? No, FT2 usually means Close[t+2])
            # Script analyze used: (c2 - c0)/c0. t+30m.
            # Let's verify data has 15m intervals.
            # Here we iterate.
            mask = sub15['datetime'] == t
            if mask.any():
                c0 = sub15.loc[mask, 'close'].values[0]
                t_next = t + pd.Timedelta(minutes=30) # 2 bars later
                mask_next = sub15['datetime'] == t_next
                if mask_next.any():
...
def run_002():
    print("::: RUN 002: FP KILL (AAVEUSDT) :::")
    ...
    # Copy Rules
    for f in ["aaveusdt_fail_rules.json", "aaveusdt_intraday_cancel_rules.json", "aaveusdt_success_corridor.json"]:
        shutil.copy(RUN_001 / "rules" / f, RUN_002 / "rules" / f)
        
    # Update Trigger Map
    trig_map = {
        "priority": "STANDARD",
        "trigger_ft2_min": 0.75,
        "rule_note": "AAVE_SPECIFIC: Avg FollowThrough2 >= 0.75% (Momentum Filter)"
    }
    
    with open(RUN_002 / "rules/aaveusdt_trigger_map.json", "w") as f:
        json.dump(trig_map, f, indent=4)
        
    # Load Data
    bundle = utils_io.get_data_bundle("AAVEUSDT")
    ...
    # Load Rules
    with open(RUN_002 / "rules/aaveusdt_fail_rules.json") as f: fail = json.load(f)
    with open(RUN_002 / "rules/aaveusdt_success_corridor.json") as f: corr = json.load(f)
    with open(RUN_002 / "rules/aaveusdt_intraday_cancel_rules.json") as f: cancel = json.load(f)
    ...
    for idx, row in df_test.iterrows():
        ...
        if daily_gate and not is_canceled:
            t_count, t_burst, t_ft2 = get_15m_metrics(df15, target_date)
            # LOGIC: Avg FT2 >= 0.75
            if t_ft2 >= 0.75:
                trig_ok = True
            else:
                trig_reason = f"TRIG_FAIL (FT2={t_ft2:.2f}%)"
                if t_count == 0: trig_reason = "TRIG_FAIL (No Triggers)"
    ...
    df_res[v17_schema.V17_COLUMNS].to_csv(RUN_002 / "reports/AAVEUSDT_V17_TEST_DAILY.csv", index=False)
    df_res[df_res['Decision']=='NET_ADAY'][v17_schema.V17_COLUMNS].to_csv(RUN_002 / "reports/AAVEUSDT_NET_ADAY_REPORT.csv", index=False)
    df_log.to_csv(RUN_002 / "reports/AAVEUSDT_SIMULATION_STAGE_LOG.csv", index=False)
    
    # Audit MD
    try:
        r1 = pd.read_csv(RUN_001 / "reports/AAVEUSDT_NET_ADAY_REPORT.csv")
    ...
    md = "# AAVEUSDT RUN 002 AUDIT (FT2>=0.75%)\n\n"
    ...
    with open(RUN_002 / "reports/AAVEUSDT_FP_KILL_AUDIT.md", "w") as f:
        f.write(md)
