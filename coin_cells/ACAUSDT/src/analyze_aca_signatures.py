...
def get_4h_regime():
    ...
    for d in all_dates:
        ...
        if len(day_candles) > 0:
            ...
            elif day_candles['STATE_RIBBON_SUPPRESSION_UP'].any(): ribbon_state = "SUPPRESSION_UP"
        ...
        if len(day_candles) > 0:
            ...
            trend_higher_low = day_close > day_open
...
def produce_verdict(df_15, df_4h):
    ...
    md = "# ACA GLOBAL FAKEOUT VERDICT\n\n"
    if rules:
        md += "## DISTINGUISHABLE\n"
        md += "ACA separates TP from FPs with:\n"
    ...
    with open(OUT_DIR / "ACA_VERDICT.md", "w") as f:
        f.write(md)
...
if __name__ == "__main__":
    ...
    df15.to_csv(OUT_DIR / "ACA_FP_TP_15M_SIGNATURES.csv", index=False)
    df4h.to_csv(OUT_DIR / "ACA_4H_REGIME_COMPARISON.csv", index=False)
