...
BASE_DIR = Path("/Users/alisaglam/TezaverMac/coin_cells/ACAUSDT")
    ...
    if count > 0:
        ...
        for t in times:
            ...
            if mask.any():
                ...
                t_next = t + pd.Timedelta(minutes=30)
...
def run_002():
    ...
    # Copy Rules
    for f in ["acausdt_fail_rules.json", "acausdt_intraday_cancel_rules.json", "acausdt_success_corridor.json"]:
    ...
    trig_map = {
        "priority": "STANDARD",
        "trigger_ft2_min": 0.0,
        "rule_note": "ACA_PARTIAL_FIX: FT2 > 0.0 (Kills 2/3 FPs, 11-07 Remains)"
    }
    
    with open(RUN_002 / "rules/acausdt_trigger_map.json", "w") as f:
        json.dump(trig_map, f, indent=4)
        
    # Load Data
    bundle = utils_io.get_data_bundle("ACAUSDT")
    ...
    # Load Rules
    with open(RUN_002 / "rules/acausdt_fail_rules.json") as f: fail = json.load(f)
    with open(RUN_002 / "rules/acausdt_success_corridor.json") as f: corr = json.load(f)
    with open(RUN_002 / "rules/acausdt_intraday_cancel_rules.json") as f: cancel = json.load(f)
    ...
    df_res[v17_schema.V17_COLUMNS].to_csv(RUN_002 / "reports/ACAUSDT_V17_TEST_DAILY.csv", index=False)
    df_res[df_res['Decision']=='NET_ADAY'][v17_schema.V17_COLUMNS].to_csv(RUN_002 / "reports/ACAUSDT_NET_ADAY_REPORT.csv", index=False)
    df_log.to_csv(RUN_002 / "reports/ACAUSDT_SIMULATION_STAGE_LOG.csv", index=False)
    
    # Audit MD
    try:
        r1 = pd.read_csv(RUN_001 / "reports/ACAUSDT_NET_ADAY_REPORT.csv")
    ...
    md = "# ACAUSDT RUN 002 AUDIT (FT2>0.0)\n\n"
    ...
    if tp_kept > 0 and len(dropped) > 0:
        ...
        md += "\n**STATUS: FINAL_LOCKED** (Solution Verified - 2/3 Killed)"
    else:
        md += "\n**STATUS: NEEDS_REVIEW**"
        
    with open(RUN_002 / "reports/ACAUSDT_FP_KILL_AUDIT.md", "w") as f:
        f.write(md)
