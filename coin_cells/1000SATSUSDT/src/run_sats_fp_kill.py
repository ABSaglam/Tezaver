...
BASE_DIR = Path("/Users/alisaglam/TezaverMac/coin_cells/1000SATSUSDT")
...
def get_15m_metrics(df_15m, target_date):
    ...
    count = len(trigs)
    # Burst logic not needed for this rule but good to have
    burst = 0
    if count > 0:
...
def run_002():
    print("::: RUN 002: FP KILL (1000SATSUSDT) :::")
    ...
    # Copy Rules
    for f in ["1000satsusdt_fail_rules.json", "1000satsusdt_intraday_cancel_rules.json", "1000satsusdt_success_corridor.json"]:
        shutil.copy(RUN_001 / "rules" / f, RUN_002 / "rules" / f)
        
    # Update Trigger Map
    trig_map = {
        "priority": "STANDARD",
        "trigger_count_min": 11,
        "rule_note": "SATS_SPECIFIC: Count>=11 (Validates 12-02 TP, Kills 11-07 FP @ 10)"
    }
    
    with open(RUN_002 / "rules/1000satsusdt_trigger_map.json", "w") as f:
        json.dump(trig_map, f, indent=4)
        
    # Load Data
    bundle = utils_io.get_data_bundle("1000SATSUSDT")
    ...
    # Load 15m
    f15 = DATA_DIR / "1000SATSUSDT_15m.csv"
    if not f15.exists():
    ...
    # Load Rules
    with open(RUN_002 / "rules/1000satsusdt_fail_rules.json") as f: fail = json.load(f)
    with open(RUN_002 / "rules/1000satsusdt_success_corridor.json") as f: corr = json.load(f)
    with open(RUN_002 / "rules/1000satsusdt_intraday_cancel_rules.json") as f: cancel = json.load(f)
    ...
    df_res[v17_schema.V17_COLUMNS].to_csv(RUN_002 / "reports/1000SATSUSDT_V17_TEST_DAILY.csv", index=False)
    df_res[df_res['Decision']=='NET_ADAY'][v17_schema.V17_COLUMNS].to_csv(RUN_002 / "reports/1000SATSUSDT_NET_ADAY_REPORT.csv", index=False)
    df_log.to_csv(RUN_002 / "reports/1000SATSUSDT_SIMULATION_STAGE_LOG.csv", index=False)
    
    # Audit MD
    try:
        r1 = pd.read_csv(RUN_001 / "reports/1000SATSUSDT_NET_ADAY_REPORT.csv")
    ...
    md = "# 1000SATSUSDT RUN 002 AUDIT (COUNT KILL)\n\n"
    ...
    with open(RUN_002 / "reports/1000SATSUSDT_FP_KILL_AUDIT.md", "w") as f:
        f.write(md)
