...
def get_15m_metrics(df_15m, target_date):
    ...
    if 'volume' not in sub15.columns: return 0,0,0
    ...
    if count > 0:
        ...
        # Vol Peak
        vol_peak = trigs['VolRatio'].max()
            
    return count, burst, vol_peak
...
def run_003():
    print("::: RUN 003: CHEEMS SPECIFIC FP KILL (VolPeak) :::")
    ...
    # Copy Rules
    for f in ["1000cheemsusdt_fail_rules.json", "1000cheemsusdt_intraday_cancel_rules.json", "1000cheemsusdt_success_corridor.json"]:
    ...
    trig_map = {
        "priority": "STANDARD",
        "trigger_count_min": 8,
        "trigger_vol_peak_min": 3.5,
        "rule_note": "CHEEMS_SPECIFIC: Count>=8 + VolPeak>=3.5 (Separates 12-05 FP)"
    }
    
    with open(RUN_003 / "rules/1000cheemsusdt_trigger_map.json", "w") as f:
        json.dump(trig_map, f, indent=4)
        
    # Load Data
    bundle = utils_io.get_data_bundle("1000CHEEMSUSDT")
    ...
    # Load Rules
    with open(RUN_003 / "rules/1000cheemsusdt_fail_rules.json") as f: fail = json.load(f)
    with open(RUN_003 / "rules/1000cheemsusdt_success_corridor.json") as f: corr = json.load(f)
    with open(RUN_003 / "rules/1000cheemsusdt_intraday_cancel_rules.json") as f: cancel = json.load(f)
    ...
    for idx, row in df_test.iterrows():
        ...
        if daily_gate and not is_canceled:
            t_count, t_burst, t_vol = get_15m_metrics(df15, target_date)
            # LOGIC: Count >= 8 AND VolPeak >= 3.5
            if t_count >= 8 and t_vol >= 3.5:
                trig_ok = True
            else:
                trig_reason = f"TRIG_FAIL (Count={t_count}, VolPeak={t_vol:.2f})"
    ...
    df_res[v17_schema.V17_COLUMNS].to_csv(RUN_003 / "reports/1000CHEEMSUSDT_V17_TEST_DAILY.csv", index=False)
    df_res[df_res['Decision']=='NET_ADAY'][v17_schema.V17_COLUMNS].to_csv(RUN_003 / "reports/1000CHEEMSUSDT_NET_ADAY_REPORT.csv", index=False)
    df_log.to_csv(RUN_003 / "reports/1000CHEEMSUSDT_SIMULATION_STAGE_LOG.csv", index=False)
    
    # Audit MD
    try:
        r1 = pd.read_csv(RUN_001 / "reports/1000CHEEMSUSDT_NET_ADAY_REPORT.csv")
        r1_dates = set(pd.to_datetime(r1['Date'], utc=True).apply(lambda x: x.isoformat()).tolist())
    except:
        r1_dates = set()
        
    r3_dates = set(df_res[df_res['Decision']=='NET_ADAY']['Date'].tolist())
    
    dropped = r1_dates - r3_dates
    kept = r1_dates.intersection(r3_dates)
    
    md = "# 1000CHEEMSUSDT RUN 003 AUDIT (VOL PEAK KILL)\n\n"
    md += f"Run 001 Net Adays: {len(r1_dates)}\n"
    md += f"Run 003 Net Adays: {len(r3_dates)}\n\n"
    ...
    with open(RUN_003 / "reports/1000CHEEMSUSDT_FP_KILL_AUDIT.md", "w") as f:
        f.write(md)
