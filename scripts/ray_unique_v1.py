"""
RAYUSDT Unique Strategy V1 (Final)
===================================
RAY-Specific Strategy: EMA Distance Focus

Discovery: RAY's strongest discriminator is ema_dist (801% difference between rally/non-rally days)
Single ema_dist threshold provides best coverage with 100% precision.
...
def main():
    ...
    print(f"\n{'DATE':<12} | {'TYPE':<8} | {'GAIN':<6} | {'EMA':<8}")
    print("-" * 50)
    
    for idx in range(25, len(df_1d)-1):
        row = df_1d.loc[idx]
        
        # RAY UNIQUE: ema_dist >= 30%
        if row['ema_dist'] >= 30:
            ...
            print(f"{row['datetime'].date()} | {res[0]:<8} | %{res[1]:4.1f} | {row['ema_dist']:>6.1f}%")
