...
def main():
    symbol = 'ALGOUSDT'
    print(f"✅ DEEP VALUE V2 VERIFICATION: {symbol}")
    print("="*70)
    ...
    for idx in range(50, len(df)-1):
        ...
        # Rule: RSI Pocket Dip
        # 1. Significant Drop: Mom5 <= -15
        # 2. RSI Pocket: 20 <= RSI <= 28 (Avoid RSI<20 Knives)
        # 3. Rejection/Bounce: Shadow >= 0.20
        # 4. Capitulation Vol: Vol Ratio >= 0.8 (Not dead) but <= 3.0 (Not panic)
        
        cond = (
            (row['mom_5d'] <= -15) and
            (row['rsi'] >= 20) and
            (row['rsi'] <= 28) and # The Pocket
            (row['shadow_ratio'] >= 0.20) and
            (row['vol_ratio'] >= 0.8) and
            (row['vol_ratio'] <= 4.0)
        )
        ...
        if cond:
            ...
            print(f"{row['datetime'].date()} | {res_str} ({tier}) | M5:{row['mom_5d']:.1f} | R:{row['rsi']:.1f} | S:{row['shadow_ratio']:.2f} | V:{row['vol_ratio']:.1f}")
    ...
    validation_passed = (total >= 2 and hits/total >= 0.8) 
