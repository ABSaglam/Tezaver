"""
AGLDUSDT - Gizem Keşfi
=======================
Adventure Gold - Gaming/Metaverse token
Ne farklı? Keşfedelim.
...
def main():
    symbol = 'AGLDUSDT'
    print(f"🔍 {symbol} (Adventure Gold) - Gaming Token Gizemi...")
    ...
    print(f"✓ {len(rally_results)} DG rallies (train) - Gaming sektörü özel mi?")
    ...
    print("\n" + "="*70)
    print(f"🎯 {symbol} - Gaming Token DNA")
    ...
    print("\n" + "="*70)
    print("⚡ AYIRICI GÜÇ - Gaming Token Farklı mı?")
    print("="*70)
    ...
    # Özel kontrol: Pozitif mi negatif mi?
    print("\n🔬 ÖNE ÇIKAN İndikatör Detayı:")
    top_ind = discriminators[0]['indicator']
    print(f"{top_ind} rally dağılımı:")
    print(f"  Pozitif: {(rally_df[top_ind] > 0).sum()} | Negatif: {(rally_df[top_ind] < 0).sum()}")
