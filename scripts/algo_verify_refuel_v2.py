...
def main():
    symbol = 'ALGOUSDT'
    print(f"✅ RE-FUEL V2 VERIFICATION: {symbol}")
    print("="*70)
    ...
    for idx in range(50, len(df)-1):
        ...
        cond = (
            ...
            (row['mom_5d'] <= 25) and
            (row['ema_dist'] >= 10) and
            (row['vol_ratio'] >= 0.8) and # Avoiding Zombies
            (row['upper_shadow_ratio'] <= 0.26) # Ensuring Clean Close
        )
        ...
        if cond:
            ...
            print(f"{row['datetime'].date()} | {res_str} ({tier}) | M5:{row['mom_5d']:.1f} | US:{row['upper_shadow_ratio']:.2f}")
    ...
    validation_passed = (total >= 3 and hits/total >= 0.8) 
