...
def analyze_aca_tunnel_hits():
    symbol = "ACAUSDT"
    print(f"🔬 ACAUSDT - Ayaş Tüneli İsabet Analizi (Hitting vs Missing)")
    print("="*70)
    ...
    # Load Reality (Historical Rallies)
    store = RallyStore()
    db_rallies = store.list_rallies(symbol=symbol, timeframe='15m')
    # A 'Real Rally' is Tier >= SILVER
    ...
    for day_ts in df_target.index:
        ...
        if is_trend or is_ninja:
            # Check if a rally happened on this day or the next day
            hit = False
            for r_day in real_set:
                if 0 <= (r_day - day_ts).days <= 2: # Within 48 hours
                    hit = True
                    break
    ...
    if not misses.empty:
        print("❌ Fos Çıkan Bazı Tarihler:")
        for d in misses['date'].head(10):
            print(f"- {d.date()}")

if __name__ == "__main__":
    analyze_aca_tunnel_hits()
