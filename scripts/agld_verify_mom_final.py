...
def main():
    symbol = 'AGLDUSDT'
    print(f"✅ FINAL MOMENTUM VERIFICATION: {symbol}")
    print("="*70)
    ...
    for idx in range(50, len(df)-1):
        ...
        # Rule: Shadow-Capped Supernova
        # Mom5 >= 30 AND RSI >= 65 AND Vol <= 5.0 AND UpperShadow <= 0.45
        cond = (
            (row['mom_5d'] >= 30) and
            (row['rsi'] >= 65) and
            (row['vol_ratio'] <= 5.0) and
            (row['upper_shadow_ratio'] <= 0.45)
        )
        ...
        if cond:
            ...
            signals.append({
                ...
                'rsi': row['rsi'],
                'vol': row['vol_ratio'],
                'us': row['upper_shadow_ratio']
            })
            
            print(f"{row['datetime'].date()} | {res_str} ({tier}) | M5:{row['mom_5d']:.1f} | R:{row['rsi']:.1f} | V:{row['vol_ratio']:.1f} | US:{row['upper_shadow_ratio']:.2f}")
    ...
    validation_passed = (total >= 2 and hits/total >= 0.8) # Target 80%+
