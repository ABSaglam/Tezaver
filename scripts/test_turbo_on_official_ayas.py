...
def scan_ayas_official_days():
    symbol = "ALGOUSDT"
    path_1d = f"coin_cells/ALGOUSDT/data/history_1d.parquet"
    path_4h = f"coin_cells/ALGOUSDT/data/history_4h.parquet"
    path_1h = f"coin_cells/ALGOUSDT/data/history_1h.parquet"
    path_15m = f"coin_cells/ALGOUSDT/data/history_15m.parquet"
    ...
    pass_days = []
    # Loop over 1D index in 2023
    df_1d_2023 = df_1d[df_1d.index.year == 2023]
    
    for day_ts in df_1d_2023.index:
        atr = df_1d.loc[day_ts, 'atr_pct']
        # For 4H RSI, take the last RSI of the previous day or first of this day? 
        # Usually Ayaş checks the 'current' potential.
        # Let's take the RSI at midnight of this day.
        try:
    ...
    print(f"🚇 Found {len(pass_days)} days passing 'Ayaş Tüneli' in 2023.")
    ...
    for day in pass_days:
        ...
        rally_data = {'date': str(day.date()), '15m_data': day_df.to_dict('records')}
        asm = TurboSniper_v12(rally_data, df_1h)
        res = asm.run()
        results.append(res)
    ...
    for res in results:
        if res.get('entry_idx') is not None:
            ...
            print(f"⚪ {res['date']:12} | Skip | {'No signal'}")
