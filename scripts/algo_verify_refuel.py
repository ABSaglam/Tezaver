...
def main():
    symbol = 'ALGOUSDT'
    print(f"✅ FINAL RE-FUEL VERIFICATION: {symbol}")
    print("="*70)
    ...
    for idx in range(50, len(df)-1):
        ...
        # Rule: Rocket Re-Fuel
        # 1. Steady Momentum: 10 <= Mom5 <= 25
        # 2. Strong Trend: EMA Dist >= 10
        # 3. No Blow-off: RSI <= 82
        # 4. Quiet Vol: Vol Ratio <= 2.0
        ...
        if cond:
            ...
            print(f"{row['datetime'].date()} | {res_str} ({tier}) | M5:{row['mom_5d']:.1f} | E:{row['ema_dist']:.1f} | US:{row['upper_shadow_ratio']:.2f}")
