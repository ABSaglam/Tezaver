"""
Quadratic Cluster DNA Extractor (V3 - Full History)
===================================================
Extracts DNA for Diamonds from SYN, OM, MDT, RAY using ALL data.
...
def main():
    ...
    for symbol in symbols:
        print(f"Extracting FULL DNA for {symbol}...")
        ...
        if dna_matrix:
            # 10 clusters for full mastery (more granularity)
    ...
    output_path = "library/quad_cluster_archetypes_full.json"
    with open(output_path, 'w') as f:
        json.dump(all_archetypes, f, indent=2)
    
    print(f"\n✅ Full History Archetypes saved to: {output_path}")
    conn.close()
