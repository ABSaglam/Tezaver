...
Range: Entire 3-year dataset.
Logic: High DNA Match (0.98) + Weekly Bullish + 1H Sustainability.
"""
...
def main():
    with open("library/quad_cluster_archetypes_full.json", 'r') as f: 
    ...
    print("="*100)
    print(f"{'COIN':<10} | {'DATE':<12} | {'TYPE':<8} | {'GAIN':<6} | {'SCORE':<6} | {'HEAT':<4} | {'STATUS'}")
    print("-" * 100)
    ...
    for symbol in symbols:
        ...
        for idx in range(25, len(df_1d)-1):
            ...
            if score > 0.985: # Strict DNA match
                ...
                hot_hours = len(day_bars_1h[day_bars_1h['volume'] > day_bars_1h['vol_ma24'] * 1.5]) # Relaxed heat for full scan
                ...
                if f_week and f_heat and f_rsi:
                    ...
                    signals.append({'symbol': symbol, 'is_hit': is_hit})
                    status = "✅ HIT" if is_hit else "❌ FAIL"
                    print(f"{symbol:<10} | {str(dt):<12} | {res[0]:<8} | %{res[1]:4.1f} | {score:.3f} | {hot_hours:<4} | {status}")
