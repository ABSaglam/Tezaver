...
def calculate_mtf_indicators(df_4h):
    ...
    df_d['d_ema21'] = df_d['close'].ewm(span=21).mean()
    df_d['d_ema50'] = df_d['close'].ewm(span=50).mean()
    df_d['d_trend'] = (df_d['d_ema21'] / df_d['d_ema50'] - 1) * 100
    ...
    df_w['w_trend'] = (df_w['close'] / df_w['close'].shift(1) - 1) * 100 # Simple momentum
    
    # 4. Merge Back to 4H (Forward Fill)
    # We must reset index to merge
    df_4h['date'] = df_4h['datetime'].dt.date
    # For weekly, we can map by 'week check' or just reindex. 
    # Easiest is reindex with ffill, but be careful of lookahead. 
    # Use 'asof' merge logic? 
    # Simpler: ffill strictly.
    
    # Reindex Daily to 4H
    df_d_re = df_d.reindex(df_4h['datetime'], method='ffill')
    df_4h['d_rsi'] = df_d_re['d_rsi'].values
    df_4h['d_trend'] = df_d_re['d_trend'].values
    
    # Reindex Weekly to 4H
    df_w_re = df_w.reindex(df_4h['datetime'], method='ffill')
    df_4h['w_rsi'] = df_w_re['w_rsi'].values
    df_4h['w_mom'] = df_w_re['w_trend'].values
...
def main():
    ...
    # Expanded Grid
    w_rsi_mins = [40, 50, 60, 70]
    d_rsi_mins = [40, 50, 60, 70]
    h4_trend_mins = [0, 10, 20, 30] # Higher trends
    h4_vol_limits = [1.5, 2.0, 3.0, 5.0, 10.0] # Both caps and floors
    ...
    for w, d, t, v in itertools.product(w_rsi_mins, d_rsi_mins, h4_trend_mins, h4_vol_limits):
        ...
        if total > 0:
            p = h / total * 100
            # Prefer chaos if same precision? Or just update if better.
            if p >= best_perf[0] and h > 0: 
                # If equal precision, prioritise more hits
