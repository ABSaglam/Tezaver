"""
MDTUSDT Baseline Detector V1
============================
Tests baseline detection with MDT-specific 15 fine-grained archetypes.
Goal: Establish baseline precision before optimization.
"""
...
def main():
    ...
    print("="*100)
    print(f"{'DATE':<12} | {'TYPE':<8} | {'GAIN':<6} | {'SCORE':<6} | {'EMA':<8}")
    print("-" * 100)
    ...
    for idx in range(25, len(df_1d)-1):
        ...
        # Simple baseline: DNA >= 0.90, Weekly Slope > 0
        if score >= 0.90 and w_row['w_slope'] > 0:
            next_date = df_1d.loc[idx+1, 'datetime'].date()
            res = all_results.get(next_date, ('NONE', 0.0))
            is_hit = res[0] in ['DIAMOND', 'GOLD', 'SILVER']
            
            signals.append({'is_hit': is_hit, 'score': score, 'ema': row['ema_dist'], 'tier': res[0], 'gain': res[1]})
            print(f"{str(row['datetime'].date()):<12} | {res[0]:<8} | %{res[1]:4.1f} | {score:.3f} | {row['ema_dist']:.1f}%")

    df = pd.DataFrame(signals)
    print("\n" + "="*60)
    print("🎯 MDTUSDT BASELINE V1 RECAP")
    print("="*60)
    if not df.empty:
