...
def main():
    symbol = 'ALGOUSDT'
    print(f"✅ FLASH CRASH VERIFICATION: {symbol}")
    print("="*70)
    ...
    for idx in range(50, len(df)-1):
        ...
        # Rule: Flash Crash Hunter
        # 1. Deep Crash: Mom5 <= -15 (Verified)
        # 2. Daily Blood: DCH <= -8 (Verified from winners)
        # 3. Panic Volume: Vol >= 2.0 (Verified, losers are quiet)
        # 4. RSI Oversold: RSI <= 35 (Standard)
        ...
        if cond:
            ...
            print(f"{row['datetime'].date()} | {res_str} ({tier}) | M5:{row['mom_5d']:.1f} | DCH:{row['daily_ch']:.1f} | V:{row['vol_ratio']:.1f}")
