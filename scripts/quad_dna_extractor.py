"""
Quadratic Cluster DNA Extractor
===============================
Extracts DNA for Diamonds from SYN, OM, MDT, RAY.
Focuses on general archetypes to avoid over-fitting (locking).
"""
...
def main():
    ...
    for symbol in symbols:
        ...
        if dna_matrix:
            # 5 clusters per coin for a balanced view (not too broad, not too locked)
            n_clusters = 5
    ...
    output_path = "library/quad_cluster_archetypes.json"
    with open(output_path, 'w') as f:
        json.dump(all_archetypes, f, indent=2)
    
    print(f"\n✅ Archetypes for all 4 coins saved to: {output_path}")
    conn.close()
