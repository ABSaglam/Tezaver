"""
ATOMUSDT - Gizem Keşfi
=======================
Cosmos - L1 / Interop
⚠️ Sadece 4 DG Rally! (Çok az)
Veri yetersizliği riski var.
Bu coin neden hiç rally yapmıyor? Ölü dev mi?
Yoksa çok nadir ve büyük mü hareket ediyor?
...
def main():
    symbol = 'ATOMUSDT'
    print(f"🔍 {symbol} (Cosmos) - Uyuyan Dev Gizemi...")
    ...
    print("\n" + "="*70)
    print(f"🎯 {symbol} - ATOM DNA")
    print(f"Rally: {len(rally_df)} | Normal: {len(non_rally_df)}")
    print("="*70)
    
    if len(rally_df) < 5:
        print("⚠️ ÇOK AZ VERİ! İstatistikler yanıltıcı olabilir.")
    ...
    print("\n" + "="*70)
    print("⚡ AYIRICI GÜÇ - ATOM Farklı mı?")
    print("="*70)
