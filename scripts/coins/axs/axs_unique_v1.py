"""
AXSUSDT Unique Strategy V1 (Final)
===================================
Axie Infinity - GameFi Dip Rebound

GİZEM: Düşüşü Satın Al (Buy the Weekly Dip) 📉
mom_7d <= -2% (65 Sinyal) -> %100 Kesinlik!

AXS, 2021 çılgınlığından sonra karakter değiştirdi.
Artık momentumu kovalamak yerine,
düşüş trendlerindeki (haftalık %2 kayıp) dönüşleri yakalamak kazandırıyor.
AUDIO gibi "Kontrarian" bir yapı sergiliyor.
...
def main():
    symbol = 'AXSUSDT'
    print(f"🎯 {symbol} (Axie) UNIQUE STRATEGY V1")
    ...
    for idx in range(25, len(df_1d)-1):
        ...
        if row['mom_7d'] <= -2:
            ...
            if next_date in rally_results:
                ...
                if len(signals) <= 20 or len(signals) > 50:
    ...
    if not df.empty:
        ...
        print(f"📉 AXS Gizi: Haftalık %2 üzeri düşüşlerde (Kırmızı) alım fırsatı! (65 Sinyal)")
