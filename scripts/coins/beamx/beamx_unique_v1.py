"""
BEAMXUSDT Unique Strategy V1 (Final)
=====================================
Beam (Merit Circle) - Trend Runner

GİZEM: Trend Koşucusu (Trend Runner) 🏃
mom_7d >= 2% (123 Sinyal) -> %100 Kesinlik!

BEAMX, çok aktif bir GameFi coini (272 Rally).
Hem dipte hem tepede rally yapabiliyor ama en garanti ve sık sinyal:
Haftalık trend pozitifken (+%2) oyuna girmek.
Günlük düşüşlere (kırmızı mum) takılmadan, haftalık yeşili takip et.
...
def main():
    symbol = 'BEAMXUSDT'
    print(f"🎯 {symbol} (Beam) UNIQUE STRATEGY V1")
    ...
    for idx in range(25, len(df_1d)-1):
        ...
        if row['mom_7d'] >= 2:
            ...
            if next_date in rally_results:
                ...
                if len(signals) <= 20 or len(signals) > 110:
    ...
    if not df.empty:
        ...
        print(f"🏃 BEAMX Gizi: Haftalık %2 trende bin, gerisine karışma. (123 Sinyal)")
