"""
ALICEUSDT Threshold Sweep
==========================
My Neighbor Alice (Gaming)
Pattern: Mixed / Dual Personality
daily_ch: Pozitif ortalama (+2.27%) ama 14 (+), 11 (-) dağılım.
mom_5d: 13 (+), 12 (-)

Gaming token (AGLD) saf momentum idi. ALICE karışık.
Test: Momentum vs Dip-Buying vs Hybrid
"""
...
def main():
    ...
    print("\n" + "="*70)
    print("🔬 ALICE Strateji Testleri")
    print("="*70)
    
    # Test 1: Momentum (mom_5d)
    print("\n📊 Test 1: Momentum (mom_5d):")
    ...
    # Test 2: Momentum (mom_7d)
    print("\n📊 Test 2: Long Momentum (mom_7d):")
    for thresh in [5, 10, 15, 20]:
        signals = []
        for idx in range(25, len(df_1d)-1):
            row = df_1d.loc[idx]
    ...
    # Test 4: Hybrid (OR logic)
    print("\n📊 Test 4: Hybrid (mom_5d OR dip):")
    for m in [15, 20]:
        for d in [-5, -8]:
            signals = []
            for idx in range(25, len(df_1d)-1):
                row = df_1d.loc[idx]
                if row['mom_5d'] >= m or row['daily_ch'] <= d:
                    next_date = df_1d.loc[idx+1, 'datetime'].date()
                    if next_date in rally_results:
                        tier, gain = rally_results[next_date]
                        is_hit = tier in ['DIAMOND', 'GOLD', 'SILVER']
            ...
            if signals:
                ...
                if prec >= 95:
                    marker = " ✅" if prec == 100 else ""
                    print(f"  mom_5d>={m}% OR daily<={d}%: {len(signals)} sinyal, {hits} hit -> {prec:.0f}%{marker}")
