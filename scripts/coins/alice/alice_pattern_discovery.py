"""
ALICEUSDT - Gizem Keşfi
========================
My Neighbor Alice - Gaming/Metaverse
Gaming token'ları (AGLD) genelde aktif olur. ALICE nasıl?
...
def main():
    symbol = 'ALICEUSDT'
    print(f"🔍 {symbol} (Gaming) - Metaverse Gizemi...")
    ...
    print(f"✓ {len(rally_results)} DG rallies (train) - Gaming token.")
    ...
    print("\n" + "="*70)
    print(f"🎯 {symbol} - Gaming/Metaverse DNA")
    ...
    print("\n" + "="*70)
    print("⚡ AYIRICI GÜÇ - ALICE Farklı mı?")
    print("="*70)
    ...
    # Özel kontrol: daily_ch ve mom_3d negatif mi?
    print("\n🔬 DAĞILIM KONTROLÜ:")
    for ind in ['mom_5d', 'daily_ch']:
        pos = (rally_df[ind] > 0).sum()
        neg = (rally_df[ind] < 0).sum()
        print(f"  {ind:<10}: {pos} Pozitif | {neg} Negatif")
