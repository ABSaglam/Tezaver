"""
ALGOUSDT Threshold Sweep
=========================
Algorand - L1 Blockchain
Pattern Discovery Bulguları:
1. mom_7d (+15k%), mom_5d (+12k%) - Çok güçlü momentum
2. daily_ch (-3.8k%) - Negatif! (-2.44% ortalama)

Hipotez: Hem momentum hem dip-buying çalışabilir.
"""
...
def main():
    symbol = 'ALGOUSDT'
    print(f"🔬 {symbol} (Algorand) Threshold Sweep")
    ...
    print("\n" + "="*70)
    print("🔬 ALGO Strateji Testleri")
    print("="*70)
    
    # Test 1: Momentum (mom_7d - En güçlü discriminator)
    ...
    print("\n📊 Test 4: Hybrid (mom_7d OR dip):")
    for m in [20, 25]:
        for d in [-8, -10]:
            signals = []
            for idx in range(25, len(df_1d)-1):
                row = df_1d.loc[idx]
                if row['mom_7d'] >= m or row['daily_ch'] <= d:
                    next_date = df_1d.loc[idx+1, 'datetime'].date()
                    if next_date in rally_results:
                        tier, gain = rally_results[next_date]
                        is_hit = tier in ['DIAMOND', 'GOLD', 'SILVER']
            ...
            if signals:
                ...
                if prec >= 95:
                    marker = " ✅" if prec == 100 else ""
                    print(f"  mom_7d>={m}% OR daily<={d}%: {len(signals)} sinyal, {hits} hit -> {prec:.0f}%{marker}")
