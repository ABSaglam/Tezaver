"""
APTUSDT Threshold Sweep
========================
Aptos - L1 Blockchain
Pattern Discovery:
daily_ch: -2.67% avg (-2864% fark!) -> Dip King? 📉
mom_3d: -2.93% avg (-1155% fark)
mom_7d: -2.96% avg (-564% fark)

ALGO (L1) momentumcuydu, APT (L1) dipçi çıktı.
Test: Dip Buying vs Momentum (Kontrol amaçlı)
"""
...
def main():
    symbol = 'APTUSDT'
    print(f"🔬 {symbol} (Aptos) Threshold Sweep - Dip Focus")
    ...
    print("\n" + "="*70)
    print("🔬 APT Strateji Testleri")
    print("="*70)
    
    # Test 1: Daily Dip (daily_ch negatif - Pattern Lideri)
    # Fark çok büyük (-2864%), düşük eşikler bile çalışabilir.
    ...
    # Test 2: Short Dip Momentum (mom_3d negatif)
    ...
    # Test 3: Momentum Check (Control)
    print("\n📊 Test 3: Momentum Check (mom_7d positive):")
    for thresh in [5, 10]:
        signals = []
        for idx in range(25, len(df_1d)-1):
            row = df_1d.loc[idx]
