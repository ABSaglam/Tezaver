...
================================
OM's key discriminator: ema_dist (8404% difference!)
"""
...
def main():
    ...
    # ema_dist sweep (OM's best discriminator)
    print("\n📊 ema_dist Sweep:")
    ...
    for e in [15, 20, 25]:
        for m in [15, 20, 25, 30]:
            ...
            for idx in range(25, len(df_1d)-1):
                row = df_1d.loc[idx]
                if row['ema_dist'] >= e and row['mom_5d'] >= m:
                    next_date = df_1d.loc[idx+1, 'datetime'].date()
                    res = all_results.get(next_date, ('NONE', 0.0))
                    is_hit = res[0] in ['DIAMOND', 'GOLD', 'SILVER']
                    signals.append(is_hit)
            ...
            if signals:
                ...
                if prec >= 90:
                    marker = " ✅" if prec == 100 else " 🔶"
                    print(f"  ema>={e}%, mom_5d>={m}%: {len(signals)} sig, {hits} hit -> {prec:.1f}%{marker}")
    
    # ema_dist + daily_ch combined
    print("\n📊 ema_dist + daily_ch Combined:")
    for e in [15, 20, 25]:
        for d in [10, 15, 20]:
            signals = []
