...
def main():
    ...
    # 9. REFINING MOMENTUM (Mom5 >= 25)
    print("\n" + "="*80)
    print("🔍 REFINING CANDIDATE H1 (Mom5 >= 25)")
    print("="*80)
    
    # H1: Mom5 >= 25
    h1_hits = [r for idx, r in hits_df.iterrows() if r['mom_5d'] >= 25]
    h1_fails = [r for idx, r in fails_df.iterrows() if r['mom_5d'] >= 25]
    total = len(h1_hits) + len(h1_fails)
    if total > 0:
        print(f"Base H1 (Mom5 >= 25): {len(h1_hits)}/{total} -> {len(h1_hits)/total*100:.1f}%")
    ...
    # Mom7 Confirmation
    for m in [20, 25, 30]:
        h_cnt = sum(1 for row in h1_hits if row['mom_7d'] >= m)
        f_cnt = sum(1 for row in h1_fails if row['mom_7d'] >= m)
        tot = h_cnt + f_cnt
        if tot > 0: print(f"    + mom_7d >= {m}: {h_cnt}/{tot} -> {h_cnt/tot*100:.1f}%")

    # EMA Dist Confirmation
    for e in [20, 30, 40]:
        h_cnt = sum(1 for row in h1_hits if row['ema_dist'] >= e)
        f_cnt = sum(1 for row in h1_fails if row['ema_dist'] >= e)
        tot = h_cnt + f_cnt
        if tot > 0: print(f"    + ema_dist >= {e}: {h_cnt}/{tot} -> {h_cnt/tot*100:.1f}%")
        
    # RSI
    for r in [60, 65, 70]:
        h_cnt = sum(1 for row in h1_hits if row['rsi'] >= r)
        f_cnt = sum(1 for row in h1_fails if row['rsi'] >= r)
        tot = h_cnt + f_cnt
        if tot > 0: print(f"    + rsi >= {r}: {h_cnt}/{tot} -> {h_cnt/tot*100:.1f}%")
        
    # Vol Cap
    for v in [3.0, 4.0, 5.0]:
        h_cnt = sum(1 for row in h1_hits if row['vol_ratio'] <= v)
        f_cnt = sum(1 for row in h1_fails if row['vol_ratio'] <= v)
        tot = h_cnt + f_cnt
        if tot > 0: print(f"    + vol <= {v}: {h_cnt}/{tot} -> {h_cnt/tot*100:.1f}%")
