"""
8-Coin Mega Cluster Detector V6 (Final)
========================================
GHOST: DNA >= 1.0, EMA >= 10%, Weekly Slope > 0, DailyCh >= 5% (to avoid stall)
...
def main():
    with open("library/mega_cluster_archetypes.json", 'r') as f: archetypes = json.load(f)
    symbols = list(archetypes.keys())
    ...
    print("="*110)
    print(f"{'COIN':<12} | {'DATE':<12} | {'PATH':<10} | {'TYPE':<8} | {'GAIN':<6} | {'SCORE':<6}")
    print("-" * 110)
    ...
    for symbol in symbols:
        ...
        for idx in range(26, len(df_1d)-1):
            ...
            # PATH 1: GHOST (DNA >= 1.0, EMA >= 10%, DailyCh >= 5% to avoid stalling)
            ...
            # PATH 2: MOMENTUM (DNA >= 0.85, DailyCh >= 15%, EMA >= 20%, PrevDayCh >= 5%)
            ...
            if path:
                ...
                print(f"{symbol:<12} | {str(row['datetime'].date()):<12} | {path:<10} | {res[0]:<8} | %{res[1]:4.1f} | {score:.3f}")

    df = pd.DataFrame(signals)
    print("\n" + "="*60)
    print("🎯 MEGA CLUSTER V6 FINAL RECAP: 8 COINS")
    print("="*60)
    if not df.empty:
