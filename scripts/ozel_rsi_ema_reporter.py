#!/usr/bin/env python3
"""
TEZAVER ÖZEL RAPOR OLUŞTURUCU v1 (RSI-EMA Cross Ribbon)
Tetik: RSI-EMA'nın Ribbon'daki TÜM çizgileri (20-55 arası) yukarı kestiği an.
"""
...
# ==========================================
# 🛠️ KULLANICI AYARLARI (BURAYI DÜZENLEYİN)
# ==========================================
...
OUTPUT_FILE = "ozel_rsi-ema_01.md"
...
def format_row_data(r, t, i, idx):
    ...
    if i == 0:
        return [str(idx), sym_str, p_s, c_s, t['time'], sig_val, t['m_tr'], pos_icon, ang_emoji, ra_f, val_icon, trig_val, tier_icon, pk, bar_val, next_val, nn_val, cgs_f, atr_f, vh_f, ve_f, v_i, vd_f, vm_f]
    else:
        return ["", "", "", "", t['time'], sig_val, t['m_tr'], pos_icon, ang_emoji, ra_f, val_icon, trig_val, tier_icon, pk, bar_val, next_val, nn_val, cgs_f, atr_f, vh_f, ve_f, v_i, vd_f, vm_f]

def run_custom_report():
    print(f"Özel Rapor Oluşturuluyor (RSI-EMA Cross Ribbon)...")
    print(f"Tarih Aralığı: {TARGET_START_DATE} -> {TARGET_END_DATE}")

    symbols = ["SYNUSDT"] # [d for d in os.listdir(COIN_CELLS_DIR) if os.path.isdir(os.path.join(COIN_CELLS_DIR, d))]
    ...
    for symbol in symbols:
        try:
            ...
            for p in ribbon_periods:
                # RSI üzerinde EMA hesaplanmalı, fiyat değil.
            ...
            # === TRIGGER LOGIC: RSI-EMA CROSSES ALL RIBBON LINES (UPWARD) ===
            # RSI-EMA > all_ribbon_lines AND prev_RSI-EMA <= at_least_one_ribbon_line
            
            # Max of ribbons at each point
            ribbon_max = df_15m[rsi_ribbon_cols].max(axis=1)
            
            rsi_ema = df_15m['rsi_ema']
            
            # Condition 1: RSI-EMA is above ALL ribbons now
            cond_now = rsi_ema > ribbon_max
            
            # Condition 2: RSI-EMA was below OR equal to at least one ribbon previously
            # Effectively, it just crossed the highest ribbon line
            # We can detect this by: shift(1) was Not(cond_now)
                ...
                try:
                    ...
                    for i in day_indices:
                        ...
                        if is_trigger:
                            ...
                            if symbol == "SYNUSDT" and current.strftime('%Y-%m-%d') == "2026-01-30":
                                print(f"DEBUG: Found SYN trigger at {df_15m.index[i].strftime('%H:%M')} | Peak: {peak_curr:.2f} | ADX: {d_adx:.2f} | ATR: {d_atr:.2f}")
                                print(f"   Metrics: v_idx={v_idx:.2f}, v_hyb={v_hyb:.2f}, v_eff={v_eff:.2f}, v_mom={v_mom:.2f}, rsi={df_15m['rsi'].values[i]:.2f}")
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write(f"# TEZAVER ÖZEL RAPOR (RSI-EMA Cross Ribbon)\n")
        f.write(f"Tarih: {TARGET_START_DATE} - {TARGET_END_DATE}\n")
        f.write(f"Tetik: RSI-EMA'nın Ribbon içerisindeki tüm çizgileri (20-55) yukarı kestiği an.\n")
        
        hdrs = ["NO", "SYM", "MAX", "CLOSE", "TIME", "SIG", "TREND", "POS", "ANG", "R-Ang", "VAL", "P", "TIER", "P-21", "BAR", "NEXT", "N-1", "CGS", "ATR%", "Vrsi", "VBoy", "V100", "V21", "V-Mom"]
