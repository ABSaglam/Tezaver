...
def main():
    ...
    # Expanded Grid for ALICE (She might be more volatile)
    w_rsi_mins = [30, 40, 50, 60] # Wider range
    d_rsi_mins = [30, 40, 50, 60, 70]
    h4_trend_mins = [-5, 0, 5, 10, 20] # Allow dips
    h4_vol_limits = [1.5, 2.0, 3.0, 5.0, 10.0]
    ...
    for w, d, t, v in itertools.product(w_rsi_mins, d_rsi_mins, h4_trend_mins, h4_vol_limits):
        ...
        if total > 0:
            ...
            if p > best_perf[0]:
                ...
                if p == 100 and h >= 3:
                     print(f"💎 FOUND STRONG QUIET KEY: {best_rule} -> {h}/{total}")
