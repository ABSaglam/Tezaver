"""
MDTUSDT Perfect Detector V2
===========================
Rules: DNA >= 0.90, Weekly Slope > 0, (EMA >= 15% OR Daily >= 10%)
Target: 100% Precision
"""
...
def main():
    symbol = 'MDTUSDT'
    with open("library/mdt_fine_archetypes.json", 'r') as f: archetypes = json.load(f)
    ...
    print("="*100)
    print(f"{'DATE':<12} | {'TYPE':<8} | {'GAIN':<6} | {'SCORE':<6} | {'EMA':<8} | {'DAILY':<8}")
    print("-" * 100)
    ...
    for idx in range(25, len(df_1d)-1):
        ...
        score = max([calculate_dna_score(norm_dna, np.array(v)) for v in archetypes_list])
        
        w_row = df_1w[df_1w['datetime'] <= row['datetime']].iloc[-1]
        
        # Perfect V2: DNA >= 0.90, Weekly > 0, (EMA >= 15% OR Daily >= 10%)
        if score >= 0.90 and w_row['w_slope'] > 0 and (row['ema_dist'] >= 15.0 or row['daily_ch'] >= 10.0):
            next_date = df_1d.loc[idx+1, 'datetime'].date()
            res = all_results.get(next_date, ('NONE', 0.0))
            is_hit = res[0] in ['DIAMOND', 'GOLD', 'SILVER']
            
            signals.append({'is_hit': is_hit, 'score': score, 'ema': row['ema_dist'], 'daily': row['daily_ch'], 'tier': res[0], 'gain': res[1]})
            print(f"{str(row['datetime'].date()):<12} | {res[0]:<8} | %{res[1]:4.1f} | {score:.3f} | {row['ema_dist']:.1f}% | {row['daily_ch']:.1f}%")

    df = pd.DataFrame(signals)
    print("\n" + "="*60)
    print("🎯 MDTUSDT PERFECT V2 RECAP")
    print("="*60)
    if not df.empty:
        print(f"Total Signals: {len(df)}")
        print(f"Precision: {(df['is_hit'].mean()*100):.1f}%")
        print(f"\nHits: {df['is_hit'].sum()} | Fails: {len(df) - df['is_hit'].sum()}")
