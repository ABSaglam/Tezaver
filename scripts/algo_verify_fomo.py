...
def main():
    symbol = 'ALGOUSDT'
    print(f"✅ FOMO TREND VERIFICATION: {symbol}")
    print("="*70)
    ...
    for idx in range(50, len(df)-1):
        ...
        # Rule: FOMO Trend
        # 1. Strong Trend: EMA Dist >= 25 (Already flying)
        # 2. Strong Mom: Mom5 >= 8 (Positive)
        # 3. Strong RSI: RSI >= 70 (Overbought is good)
        # 4. Momentum Pause: Mom5 <= 15 (Not vertical yet) - Buying the pause
        ...
        if cond:
            ...
            print(f"{row['datetime'].date()} | {res_str} ({tier}) | E:{row['ema_dist']:.1f} | M:{row['mom_5d']:.1f}")
