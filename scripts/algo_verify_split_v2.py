...
def main():
    symbol = 'ALGOUSDT'
    print(f"✅ GOLDEN SPLIT V2 VERIFICATION: {symbol}")
    print("="*70)
    ...
    for idx in range(50, len(df)-1):
        ...
        # Deep drops only + PANIC VOLUME
        cond_dip = (
            (row['mom_5d'] <= -15) and
            (row['rsi'] <= 30) and
            (row['daily_ch'] <= -5) and
            (row['vol_ratio'] >= 1.5) and  # Require Panic Volume
            (row['vol_ratio'] <= 5.0)      # But not infinite
        )
        
        # Rule 2: The Golden Trend (Steady Uptrend)
        # Strong trend + CONTROLLED VOLUME
        cond_trend = (
            (row['mom_5d'] >= 5) and 
            (row['ema_dist'] >= 20) and 
            (row['rsi'] >= 65) and
            (row['vol_ratio'] >= 0.5) and  # Not dead
            (row['vol_ratio'] <= 1.8)      # Not blow-off
        )
        ...
        if cond_dip or cond_trend:
            ...
            print(f"{row['datetime'].date()} | {res_str} ({tier}) | {type_str} | M5:{row['mom_5d']:.1f} | V:{row['vol_ratio']:.1f} | DCH:{row['daily_ch']:.1f}")
