...
"""
ALICEUSDT Verification (Final Rule)
==================================
Rule: ema_dist 20-50 AND vol_ratio 1.5-4.0 AND rsi >= 70 AND mom_5d >= 10
"""
...
def main():
    ...
    for idx in range(50, len(df)-1):
        ...
        # FINAL RULE
        if (row['ema_dist'] >= 20 and row['ema_dist'] <= 50 and 
            row['vol_ratio'] >= 1.5 and row['vol_ratio'] <= 4.0 and
            row['rsi'] >= 70 and
            row['mom_5d'] >= 10):
