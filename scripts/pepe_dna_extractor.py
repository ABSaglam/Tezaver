"""
PEPEUSDT DNA Extractor
======================
Extracts 2-day DNA sequences for all PEPE Diamond rallies.
Generates specialized DNA archetypes for PEPE.
"""
...
def main():
    ...
    for (raw_data,) in rallies:
        ...
        # Get the 2 days leading up to the start_time (inclusive of signal day)
    ...
    if not dna_matrix:
        print("No DNA data found for PEPE.")
        return

    # Clustering PEPE Diamonds into 5 Archetypes (Meme coins have diverse moves)
    ...
    for i in range(n_clusters):
        ...
        if cluster_points:
            ...
            archetypes[f"PEPE_ARCH_{i}"] = archetype_dna.reshape(2, 5).tolist()

    output_path = "library/pepe_dg_archetypes_v1.json"
    os.makedirs("library", exist_ok=True)
    with open(output_path, 'w') as f:
        json.dump(archetypes, f, indent=2)
    
    print(f"\n✅ {len(dna_matrix)} PEPE Diamonds analyzed.")
    print(f"✅ {n_clusters} Archetypes saved to: {output_path}")
