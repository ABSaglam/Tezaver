"""
PEPEUSDT Ultimate Committee Detector V5
========================================
Combination of:
1. 'The Staircase' (V4) - Accumulation confirmed.
2. 'The Flash' (Extreme 1H Pulse + BB Squeeze).
3. 'The Mirror' (1.000 DNA Identity Match).

Goal: 100% Precision, 6+ Signals.
"""
...
def main():
    ...
    for idx in range(25, len(df_1d)-1):
        ...
        # Expert 1: THE STAIRCASE (Previous V4 Success)
        v0 = row_1d['volume']
        v1 = df_1d.loc[idx-1, 'volume']
        v2 = df_1d.loc[idx-2, 'volume']
        g2 = v1 / (v2 or 1)
        is_staircase = (best_score > 0.95) and (g2 > 1.1) and (w_row['macd_slope'] > 0) and (b4['rsi'] < 85)
        
        # Expert 2: THE FLASH (Extreme 1H + BB Squeeze)
        day_bars_1h = df_1h[df_1h['datetime'].dt.date == row_1d['datetime'].date()]
        f2_pulse = not day_bars_1h[(day_bars_1h['volume'] > day_bars_1h['vol_ma24'] * 12.0) & (day_bars_1h['price_ch'] > 12.0)].empty
        is_flash = (row_1d['bb_sq'] < 0.75) and f2_pulse and (best_score > 0.60)
        
        # Expert 3: THE MIRROR (Pure Identity Match)
        is_mirror = (best_score > 0.999) and (w_row['macd_slope'] > 0)

        if is_staircase or is_flash or is_mirror:
            path_label = "STAIRCASE" if is_staircase else ("FLASH" if is_flash else "MIRROR")
