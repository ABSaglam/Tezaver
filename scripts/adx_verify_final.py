...
"""
ADXUSDT Verification (Aggressive Momentum)
=========================================
Rule: 
  - mom_5d >= 25
  - mom_7d >= 20
...
def main():
    ...
    for idx in range(50, len(df)-1):
        ...
        # FINAL RULE
        # Aggressive Momentum + Anti-Blowoff Cap
        # Filter the 2024-03-30 failure (EMA=44.9)
        if (row['mom_5d'] >= 25 and 
            ...
            if is_hit:
                 ...
                 print(f"{current_date} | HIT | M5:{row['mom_5d']:.1f}% | M7:{row['mom_7d']:.1f} | E:{row['ema_dist']:.1f} | {tier}")
            else:
                 print(f"{current_date} | FAIL | M5:{row['mom_5d']:.1f}% | M7:{row['mom_7d']:.1f} | E:{row['ema_dist']:.1f}")
