...
def scan_aca_multi_year():
    symbol = "ACAUSDT"
    print(f"🕵️ Multi-Year Audit for {symbol} (2023-2025)")
    print("="*80)
    ...
    # Identify PASS days in 2023, 2024, 2025
    pass_days = []
    df_1d_target = df_1d[df_1d.index.year.isin([2023, 2024, 2025])]
    
    for day_ts in df_1d_target.index:
    ...
    print(f"🚇 Found {len(pass_days)} Ayaş Tüneli days in 2023-2025.")
    ...
    if trades:
        for yr in [2023, 2024, 2025]:
            ...
            yr_trades = [r for r in yr_results if r.get('entry_idx') is not None]
            if yr_results:
                yr_pnl = sum(t['pnl'] for t in yr_trades)
                yr_wr = sum(1 for t in yr_trades if t['pnl'] > 0) / len(yr_trades) * 100 if yr_trades else 0
                print(f"{yr} | Days: {len(yr_results):2} | Trades: {len(yr_trades):2} | PnL: {yr_pnl:+.2f}% | WR: {yr_wr:.1f}%")
        ...
        print("No trades taken in the target period.")
