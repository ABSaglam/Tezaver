"""
Quad Cluster Detector V8
========================
GHOST: DNA >= 1.0, EMA >= 10%, DailyCh >= 5%, Weekly Slope > 0 (100% proven)
MOMENTUM: DNA >= 0.85, DailyCh >= 15%, EMA >= 20%, PrevDayCh >= 5% (100% proven)
SURGE: DNA >= 0.60, Vol >= 4x, DailyCh >= 20%, Weekly Slope > 0 (92% proven)
"""
...
def main():
    ...
    for symbol in symbols:
        ...
        for idx in range(26, len(df_1d)-1):
            ...
            # PATH 1: GHOST (DNA >= 1.0) - 100% precision
            ...
            # PATH 2: MOMENTUM (DNA >= 0.85) - 100% precision
            elif score >= 0.85 and row['daily_ch'] >= 15.0 and row['ema_dist'] >= 20.0 and prev_row['daily_ch'] >= 5.0 and w_row['w_slope'] > 0:
                path = "MOMENTUM"
            
            # PATH 3: SURGE (Vol >= 4x, Daily >= 20%) - 92% precision
            elif score >= 0.60 and row['vol_ratio'] >= 4.0 and row['daily_ch'] >= 20.0 and w_row['w_slope'] > 0:
                path = "SURGE"
