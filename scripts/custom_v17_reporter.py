#!/usr/bin/env python3
"""
TEZAVER ÖZEL RAPOR OLUŞTURUCU v1 (Based on v17)
Belirtilen tarih ve kriterlere göre özel liste oluşturur.
"""
...
# 1. Hangi tarihler için rapor istiyorsunuz?
# Format: "YYYY-MM-DD"
TARGET_START_DATE = "2026-01-15"
TARGET_END_DATE   = "2026-01-31"

# 2. Hangi özelliklere sahip coinleri istiyorsunuz? (Filtreler)
# Devre dışı bırakmak için değeri None yapın.
FILTERS = {
    # Minumum Tier (None, '🥉', '🥈', '🥇', '💎')
    'min_tier': None, 
    
    # Minimum Güvenilirlik Skoru (CGS) yüzdesi (örn: 30)
    'min_cgs': None,
    
    # Minimum Tetik ATR yüzdesi (örn: 2.0)
    'min_atr': None,

    # Minimum Günlük ADX değeri (örn: 40) - Trend Gücü
    'min_adx': None,
    
    # Minimum Vrsi puanı (örn: 9.0)
    'min_vrsi': None,
    
    # Sadece belirli bir sinyal türü? (örn: "🚀")
    'only_rockets': False,
    
    # Sadece pozitif kapanış yapanlar? (Close > 0)
    'positive_close_only': False
}

# AYAŞ TÜNELİ KONTROLÜ
# True yapılırsa: Tünel kurallarına bakılmaksızın TÜM coinler taranır.
DISABLE_AYAS_CHECK = True

# Çıktı Dosyası Adı
OUTPUT_FILE = "ozel_14_ocak.md"
...
def run_custom_report():
    print(f"Özel Rapor Oluşturuluyor...")
    ...
    end_d = pd.Timestamp(TARGET_END_DATE) + pd.Timedelta(days=1) # Include end date fully
            ...
            while current <= end_d:
                try:
                    ...
                    if DISABLE_AYAS_CHECK:
                        is_ayas = True # Bypass Tunnel Check
                    else:
                    ...
                    for i in day_indices:
                        ...
                        if is_trigger:
                            ...
                            # Raw metrics for Portakal Sıkacağı
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write(f"# TEZAVER ÖZEL RAPOR\n")
        f.write(f"Tarih: {TARGET_START_DATE} - {TARGET_END_DATE}\n")
        f.write(f"Filtreler: {FILTERS}\n")
