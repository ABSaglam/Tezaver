...
def main():
    ...
    # Target: Today is Rally
    midnight['target'] = midnight['is_today_rally']
    
    print(f"Total Midnight Candles: {len(midnight)}")
    print(f"Total Targets: {midnight['target'].sum()}")
    
    # 4. SCORCHED EARTH BRUTE FORCE (Expanded for ACH)
    ...
    # Trend: Lower Bound
    trend_mins = [-10, -5, 0, 5, 10, 20]
    # Vol: Upper Bound (Quiet) OR Lower Bound (Chaos)
    vol_maxs = [1.5, 2.0, 3.0, 5.0, 10.0]
    # ATR: Upper Bound (Squeeze)
    atr_maxs = [2.0, 3.0, 5.0, 10.0, 100]
