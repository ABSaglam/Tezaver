        ...
        try:
            ...
            while current <= END_DATE:
                try:
                    ...
                    std_trigger_indices = np.where((rsi_vals[:-1] <= 70) & (rsi_vals[1:] > 70))[0] + 1
                    super_trigger_indices = np.where((rsi_ema_vals[:-1] <= 70) & (rsi_ema_vals[1:] > 70))[0] + 1
                    
                    # Merge and sort unique indices
                    all_trigger_indices = np.unique(np.concatenate((std_trigger_indices, super_trigger_indices)))
                    all_trigger_indices.sort()
                    
                    # Set of super indices for fast lookup
                    super_set = set(super_trigger_indices)
                    ...
                    for i in day_indices:
                        ...
                        # Check if this index is a trigger (Std or Super)
                        is_trigger = i in all_trigger_indices
                        if is_trigger:
                            is_super = i in super_set
                            # Macro Trend (4H & 1H)
                            ...
                            # RSI-EMA Symbol Logic
                            re_val = df_15m['rsi_ema'].values[i]
                            if re_val < 50: re_sym = "⚫"
                            elif re_val < 60: re_sym = "🔴"
                            elif re_val < 70: re_sym = "🟡"
                            else: re_sym = "🟢"
                            ...
                            trigger_events.append({
                                'time': time_str, 'mac_color': mac_c, 'v_index': v_idx,
                                'strat': strat_signal,
        ...
        for _, r in df.iterrows():
            ...
            if d_str != curr_d:
                ...
                if curr_d is not None and day_rows:
                     hdrs = ["NO", "SYM", "MAX", "CLOSE", "TIME", "SIG", "TREND", "R-E", "P-21", "BAR", "Vrsi", "V100", "V21", "V-Mom", "VBoy", "V-Ch", "V-Avg", "DNA"]
            ...
            for i, t in enumerate(r['triggers']):
                ...
                if p_val < 2.0:
                     pk = f"<font color='red'>{p_s}</font>"
                elif p_val < 10.0:
                     pk = p_s # Siyah/Normal
                elif p_val < 30.0:
                     pk = f"<font color='green'>{p_s}</font>"
                elif p_val < 100.0:
                     pk = f"<font color='#006400'>**{p_s}**</font>" # Koyu Yeşil (DarkGreen) + Bold
                else:
                     pk = f"<font color='#00FF00'>**{p_s}**</font>" # Fosforlu Neon
                ...
                if i==0:
                    row = [
                        ...
                        t['time'], sig_val, t['m_tr'], t['r_e'], pk, bar_val,
                        vh_f, v_i, vd_f, vm_f, ve_f, v_c, v_a, f"🖊️ {r['frek']}"
                    ]
                else:
                    row = [
                        "", "", "", "",
