...
def main():
    symbol = 'ALGOUSDT'
    print(f"✅ PINBAR AT 30 VERIFICATION: {symbol}")
    print("="*70)
    ...
    for idx in range(50, len(df)-1):
        ...
        # Rule: Pinbar at 30
        # 1. RSI not crashed: 25 <= RSI <= 35
        # 2. Huge Rejection: Shadow >= 0.35
        # 3. Mom check: Mom5 <= -10 (It is a dip)
        
        cond = (
            (row['mom_5d'] <= -10) and
            (row['rsi'] <= 35) and 
            (row['rsi'] >= 25) and
            (row['shadow_ratio'] >= 0.35)
        )
    ...
    validation_passed = (total >= 2 and hits/total >= 0.9) # High bar for small sample
    
    if validation_passed:
        print("\n✅ STRATEGY VERIFIED (>90% Precision)")
    else:
