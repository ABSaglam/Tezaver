...
"""
ACEUSDT Verification (Final Rule)
=================================
Rule: ema_dist >= 5 AND mom_3d >= 22 AND vol_ratio <= 4 AND rsi < 80
"""
...
def main():
    ...
    for idx in range(50, len(df)-1):
        ...
        if (row['ema_dist'] >= 5 and 
            ...
            if is_hit:
                 tier = rally_results[next_date][0] # Tuple index 0
                 print(f"{row['datetime'].date()} | MOM3: {row['mom_3d']:.1f}% | EMA: {row['ema_dist']:.1f}% | VOL: {row['vol_ratio']:.1f} | RSI: {row['rsi']:.1f} | {tier}")
