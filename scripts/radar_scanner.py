...
CANDIDATES_FILE = "jan28_tunel_candidates.json"
REPORT_FILE = "jan28_radar_report.md"
...
def run_radar():
    ...
    parser.add_argument("--input", required=True, help="Input candidate JSON file")
    parser.add_argument("--output", required=True, help="Output report MD file")
    parser.add_argument("--title", default="RADAR RAPORU", help="Report Title")
    ...
    if not os.path.exists(CANDIDATES_FILE):
        print(f"❌ HATA: {CANDIDATES_FILE} bulunamadı.")
        return

    with open(CANDIDATES_FILE, "r") as f:
        candidates = json.load(f)
    
    # Quick lookup for DNA
    dna_map = {c['symbol']: c['dna'] for c in candidates}
    symbols = list(dna_map.keys())
    
    print(f"📡 RADAR ({REPORT_TITLE}) BAŞLIYOR... Takip Listesi: {len(symbols)} Coin")
    ...
    for symbol in symbols:
        try:
            ...
            # Load Full Data for Intraday Analysis
            ...
            for i in day_indices:
                ...
                if is_trigger:
                    ...
                    # Stuck Check: Closed lower than trigger open
    ...
    with open(REPORT_FILE, "w", encoding="utf-8") as f:
        ...
        for _, r in df.iterrows():
            ...
            for i, t in enumerate(r['triggers']):
                ...
                if i==0:
                    # Entry Time Display
                    ent_t = dna_map.get(r['symbol'], {}).get('entry_time', '-') if isinstance(dna_map.get(r['symbol']), dict) else '-'
                    # If dna_map values are just strings (old format), handle that
                    if isinstance(dna_map.get(r['symbol']), str):
                        ent_t = "00:00" # Default for Tunnel
                    elif 'entry_time' in candidates[0]: # Check if json has it
                         # Look it up properly
                         for cand in candidates:
                             if cand['symbol'] == r['symbol']:
                                 ent_t = cand.get('entry_time', '00:00')
                    
                    # Formatting Entry Time
                    if ent_t != "00:00" and ent_t != "-":
                        ent_str = f"<font color='orange'>{ent_t}</font>"
                    else:
                        ent_str = ent_t
    ...
    print(f"✅ RADAR RAPORU HAZIR: {REPORT_FILE}")
