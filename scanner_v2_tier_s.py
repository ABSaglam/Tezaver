...
class TriggerDetector:
    ...
    def check_trigger(df_15m, df_1h):
        ...
        if (is_vol_spike or is_vol_expanding or is_mom_good) and is_holding:
            msg = "Trigger: " + ("/".join([k for k,v in {"Vol":is_vol_spike, "Range":is_vol_expanding, "Mom":is_mom_good}.items() if v]))
            return True, msg
...
class ScannerV2TierS:
    ...
    def run_simulation(self, habitat_threshold=0.58):
        ...
        print("Running Production TIER-S Simulation (Jan 2026)...")
        for current_date in tqdm(all_dates):
            for symbol, df_dict in symbol_data.items():
                if df_dict['1d'] is None or df_dict['15m'] is None: continue
                ...
                df_15m_part = df_dict['15m'][df_dict['15m']['datetime'] < current_date].tail(40)
                df_1h_part = df_dict['1h'][df_dict['1h']['datetime'] < current_date].tail(15)
                
                trigger_ok, trigger_msg = TriggerDetector.check_trigger(df_15m_part, df_1h_part)
                
                score = habitat_res['habitat_score']
                # Tiers finalized for 40-60 monthly precision target
                if score >= 0.62 and trigger_ok:
                    tier = "Tier S"
                elif score >= 0.60:
                    tier = "Tier A"
                elif score >= 0.58:
                ...
                upgraded_report.append({
                    ...
                    "Explanation": f"Elite Habitat ({tier}) with {trigger_msg}",
                    "Max %": round(max_move * 100, 2),
                    "Hit >=10%?": hit_10
                })

        if not upgraded_report:
        ...
        pd.DataFrame([summary]).to_csv(self.report_dir / "DNA_PERFORMANCE_V2.csv", index=False)
