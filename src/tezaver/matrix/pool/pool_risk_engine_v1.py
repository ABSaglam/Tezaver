...
class PoolProsecutor:
    """
    The Prosecutor.
    
    Role:
      - Indicts risky Intents.
      - Demands 'BLOCK' verdicts based on law (Limits, Policies).
      - Enforces the Kill Switch.
    """
    ...
    def indict(
        ...
        """
        Evaluate risk for each selected intent (The Indictment Process).
        """
        ...
        for item in sorted_items:
            ...
            bundle = self.registry.get(item.bundle_id)
            policy_spec = None
            ...
            # 5. Determine verdict (Indictment)
            verdict = "ALLOW"
            block_reason = None
            risk_flags: List[str] = []
...
def evaluate_risk(
    ...
    """
    Legacy wrapper for PoolProsecutor.indict.
    """
    prosecutor = PoolProsecutor(registry)
    return prosecutor.indict(selected_items, global_limits, kill_switch_triggered, risk_limiter_triggered)
