...
def render_matrix_operator_tab() -> None:
    ...
    # === Strategy Board (shown only on strategy page) ===
    if st.session_state.get("nav_page", "matrix") == "strategy":
...
def _render_live_section_v2(rows: List[ProfileBoardRow]) -> None:
    ...
    # ALLOW mapping: page_key -> set of expander titles allowed on that page
    ALLOW = {
        "matrix": {
        ...
        "strategy": {
            # Strategy Board is handled separately in parent function
        },
        "security": {
        ...
        "debug": {
            # Advanced Debug is nested in Dust & Position, shows on account page
        },
    }
    
    # Get allowed expanders for current page
    allowed_expanders = ALLOW.get(active_page, set())
    
    # Helper function for conditional rendering
    def _is_allowed(title: str) -> bool:
        """Check if expander should be rendered on current page."""
